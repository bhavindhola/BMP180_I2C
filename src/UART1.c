...
int _write(int file, char *buffer, int count)
{
    ...
    error = g_uart3.p_api->write (g_uart3.p_ctrl, (uint8_t const *) (buffer), 1);
...
int _read(int file, char *buffer, int count)
{
    ...
    error = g_uart3.p_api->read (g_uart3.p_ctrl, (uint8_t const *) (buffer), 1);
...
ssp_err_t UartOpen()
{
    ...
    // Open UART
    error = g_uart3.p_api->open (g_uart3.p_ctrl, g_uart3.p_cfg);
    if (error != SSP_SUCCESS)
        return error;
...
ssp_err_t UartClose()
{
    ...
    // Close UART
    error = g_uart3.p_api->close (g_uart3.p_ctrl);
    if (error != SSP_SUCCESS)
        return error;
