...
ssp_err_t TimerInitialize(void)
{
    ...
    error = g_timer0.p_api->open (g_timer0.p_ctrl, g_timer0.p_cfg);
...
ssp_err_t TimerSleepMs(uint32_t milliseconds)
{
    ...
    error = g_timer0.p_api->reset (g_timer0.p_ctrl);
    if (error != SSP_SUCCESS)
    {
        return error;
    }
    ...
    while (true)
    {
        error = g_timer0.p_api->counterGet (g_timer0.p_ctrl, &timerValue);
        if (error != SSP_SUCCESS)
        {
            return error;
        }
